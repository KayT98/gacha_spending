...
SELECT Items, COUNT(*) AS time_purchased, ROUND(SUM(Price), 2) AS Total -- Adds up the total money spent on this item (rounded to 2 decimals)
FROM games
GROUP BY Items -- Groups the data by each unique item
